...
export const CreatePage = () => {
  ...
  const [asyncItems, setAsyncItems] = useState<WishItem[]>([]);
  const [showColor, setShowColor] = useState(true);
  const [cardColor, setCardColor] = useState<Wishlist["color"]>("default");

  const [title, setTitle] = useState("New Wishlist")
  ...
  useEffect(() => {
    /*
    let itemsArray = rawItems.split('\n')
    let items: WishItem[] = []
    itemsArray.filter((item) => item.trim() !== "").map((item, index) => {
      items.push({id: index, name: item.trim(), checked: false})
    })
    setItems(items)
    */
  ...
  return (
    <div className={classes.root}>
      ...
      >
        <Toolbar>
          <Typography variant="h5">Create New Wishlist</Typography>
        ...
        <Container>
          <form> 
            <List>
              ...
              <ConfirmPopUp popUpButton="Cancel" color="secondary" 
              ...
              />
              <Button variant="contained" color="primary">Save as Draft</Button>
              <ConfirmPopUp popUpButton="Create" color="primary" 
                confirmButton="Confirm" confirmF={() => history.push('/')}
                rejectButton="Back" 
                title="Create new wishlist"
                content={
                  items.length === 0 ? "The Wishlist is empty, are you sure you want to create an empty wishlist?" :
                  title === "" ? "The wishlist's title is empty, are you sure you want to create a wishlist without a titile?" : 
                  `Press confirm to create this new Wishlist <${title}> with ${items.length} items: ${items.map((item)=>item.name).join(', ')}.`
                }
              />
